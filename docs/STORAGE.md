...
`MysqlAuthRepository` 使用用户名、用户 ID 和会话摘要的定向查询处理管理员身份。登录在锁定用户与所属商户后创建会话；验证通过会话、用户和商户的连接查询完成。创建账号、密码变更、密码重置、角色调整和账号停用均在锁定用户的事务内撤销对应管理会话并写入审计记录。角色调整只更新 `users.role` 与 payload，并且提交相同角色时不写库、不撤销会话。首次平台管理员创建锁定 `kmxt_meta` 的单例行来串行化初始化，不使用 MySQL advisory lock。
