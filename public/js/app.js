...
function navSection(label, items) {
  ...
  return `<div class="nav-section">
    ...
    <span class="nav-section-label">${escapeHtml(label)}</span>

    ${items.map((item) => navButton(item.id, item.icon, item.label)).join('')}
  ...
  appRoot.innerHTML = `<main class="login-page" id="main-content">
    ...
    <section class="login-panel">
      ...
      <div class="login-brand">
        ...
        <div><div class="brand-name">KMXT</div><div class="brand-caption">授权业务管理平台</div></div>
      ...
      <form class="form-stack login-form" id="login-form" autocomplete="on">
        ...
        <button class="button login-submit" type="submit" id="login-submit">登录</button>
    ...
    <div class="login-bg" aria-hidden="true">
      ...
      <canvas id="login-particles" class="bg-particles"></canvas>
    </div>

      <div class="system-map">

        <div class="map-node">${icon('building-2')}<span class="node-label">商户</span></div>

        <div class="map-node primary">${icon('boxes')}<span class="node-label">程序</span></div>

        <div class="map-node">${icon('shield-check')}<span class="node-label">授权</span></div>
...
function initLoginParticles() {
  ...
  if (!canvas || canvas.dataset.particlesReady) return;
  canvas.dataset.particlesReady = '1';

  const ctx = canvas.getContext('2d');
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  const LAYERS = [
    { glow: 'rgba(255, 255, 255, 0.35)', core: 'rgba(255, 255, 255, 0.95)', glowR: 3, coreR: 1.4, step: 48, ox: 0, oy: 0 }
  ];
  ...
  function buildParticles() {
    ...
    for (const layer of LAYERS) {
      for (let x = layer.ox; x < w; x += layer.step) {
        for (let y = layer.oy; y < h; y += layer.step) {
          particles.push({ hx: x, hy: y, dx: 0, dy: 0, ph: Math.random() * Math.PI * 2, layer });
        }
      }
    }
  }
  ...
  function render(t) {
    ...
    for (const p of particles) {
      ...
      if (dist < radius && dist > 0.001) {
        const force = (1 - dist / radius) * (1 - dist / radius) * maxShift;
        tx = (dx / dist) * force;
        ty = (dy / dist) * force;
      }
      p.dx += (tx - p.dx) * 0.12;
      p.dy += (ty - p.dy) * 0.12;
      const px = p.hx + p.dx + Math.sin(t * 0.0012 + p.ph) * 1.6;
      const py = p.hy + p.dy + Math.cos(t * 0.001 + p.ph) * 1.6;
      const layer = p.layer;
      ctx.beginPath();
      ctx.arc(px, py, layer.glowR, 0, Math.PI * 2);
      ctx.fillStyle = layer.glow;
      ctx.fill();
      ctx.beginPath();
      ctx.arc(px, py, layer.coreR, 0, Math.PI * 2);
      ctx.fillStyle = layer.core;
      ctx.fill();
    }
  }
  ...
  buildParticles();
  window.addEventListener('resize', buildParticles, { signal: ac.signal });
  window.addEventListener('mousemove', onMove, { passive: true, signal: ac.signal });
...
function renderShell() {
  ...
  const primaryViews = views.filter((item) => ['overview', 'merchants', 'applications'].includes(item.id));

  const operationsViews = views.filter((item) => ['modelArtifacts', 'announcements', 'licenses', 'onlineDevices'].includes(item.id));

  const businessViews = views.filter((item) => ['products', 'orders', 'users', 'logs'].includes(item.id));

  const contextLabel = isPlatformAdmin()

    ? (merchant ? `${merchant.name} · ${merchant.code}` : '选择商户')

    : `${merchant?.name || '商户'}${merchant?.code ? ` · ${merchant.code}` : ''}`;
  ...
  appRoot.innerHTML = `<div class="app-shell">
    ...
    <aside class="sidebar ${sidebarOpen ? 'open' : ''}" aria-label="主要导航">
      ...
      <nav class="sidebar-nav">
        ...
        ${navSection('工作台', primaryViews)}

        ${navSection('授权运营', operationsViews)}

        ${navSection('业务与审计', businessViews)}
      ...
      <div class="sidebar-user">
        ...
        <div class="user-row">
          ...
          <div class="sidebar-user-actions" aria-label="账号操作">
    ...
    <div class="main-shell">
      ...
      <header class="topbar">
        ...
        <div class="topbar-title"><span class="topbar-kicker">当前页面</span><strong>${escapeHtml(title[0])}</strong><span>${escapeHtml(title[1])}</span></div>

        <div class="inline-actions topbar-context">

          <span class="context-caption">${isPlatformAdmin() ? '数据范围' : '当前商户'}</span>

          ${merchantControl || `<span class="context-value">${escapeHtml(contextLabel)}</span>`}
